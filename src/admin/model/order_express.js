module.exports = class extends think.Model {
  ...
  async getLatestOrderExpress(orderId) {
    console.log("-----------获取订单" + orderId + "的最新状态----------");
    ...
    const orderExpress = await this.where({ order_id: orderId }).limit(1).find();
    console.log("orderExpress is:  ", orderExpress)
    if (think.isEmpty(orderExpress)) {
      console.log("没有此订单的物流信息(orderExpress isEmpty)");
      return returnExpressInfo;
    }
    if (think.isEmpty(orderExpress.shipper_code) || think.isEmpty(orderExpress.logistic_code)) {
      console.log("缺少shipper_code 或 logistic_code");
      return returnExpressInfo;
    }
    ...
    console.log("latestExpressInfo", latestExpressInfo);
    ...
    console.log("returnExpressInfo", returnExpressInfo);
    return returnExpressInfo;
  }
};
