...
module.exports = class extends Base {
   ...
   */
  async listAction() {
    const addressList = await this.model('address').where({user_id: this.ctx.state.userId}).select();
    let itemKey = 0;
  ...
  async detailAction() {
    ...
    const addressInfo = await this.model('address').where({user_id: this.ctx.state.userId, id: addressId}).find();
  ...
  async saveAction() {
    ...
    if (think.isEmpty(addressId)) {
      ...
      await this.model('address').where({id: addressId, user_id: this.ctx.state.userId}).update(addressData);
    }

    // 如果设置为默认，则取消其它的默认
    if (this.post('is_default') === true) {
      await this.model('address').where({id: ['<>', addressId], user_id: this.ctx.state.userId}).update({
        is_default: 0
      });
    }
  ...
  async deleteAction() {
    ...
    await this.model('address').where({id: addressId, user_id: this.ctx.state.userId}).delete();
