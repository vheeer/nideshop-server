...
module.exports = class extends Base {
  async listAction() {
    ...
    const list = await this.model('collect')
      ...
      .join({
        ...
        as: 'g',
        on: ['c.value_id', 'g.id']
      }).where({user_id: this.ctx.state.userId, type_id: parseInt(typeId)}).countSelect();
  ...
  async addordeleteAction() {
    ...
    const collect = await this.model('collect').where({type_id: typeId, value_id: valueId, user_id: this.ctx.state.userId}).find();
    let collectRes = null;
    let handleType = 'add';
