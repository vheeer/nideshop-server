...
module.exports = class extends Base {
  async indexAction() {
    ...
    const historyKeywordList = await this.model('search_history').distinct('keyword').where({ user_id: this.ctx.state.userId }).limit(10).getField('keyword');
  ...
  async clearhistoryAction() {
    await this.model('search_history').where({ user_id: this.ctx.state.userId }).delete();
    return this.success();
  }
};
